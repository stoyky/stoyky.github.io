...
Running FLOSS on the binary gives a long wall of strings to search through, but looking carefully
the following seemingly malicious Powershell command can be observed. It attempts to execute
a base 64 encoded blob (removed from this command for brevity):
```
...
Looking at the import address table (IAT), there are many API calls that may be abused for malicious activity, such as
RegCreateKeyA, ShellExecutaA, CreateProcessA, GetClipBoardData, there are too many to name.
Judging from the readily visible API calls, there is no indication that this binary is packed.
Running the binary through PEiD shows that the binary is unpacked (or at least unrecognized by PEiD).


## Basic Dynamic Analysis

Without internet simulation, running the executable shows a Powershell window for a brief moment, which is abnormal behavior for non-malicious Putty.
Using procmon, it is observed that the port 8443 is opened on the machine. Previously found in the strings, we decode the base64 encoded payload, followed by a Gunzip step in CyberChef, which shows the following contents:
```powershell
...
```
From the looks of it, this script opens a reverse shell on port 8443. 
If we keep an eye on Wireshark while detonating the binary, multiple DNS requests
are observed to the following domain:
```
bonus2.corporatebonusapplication.local
```
It is likely that the communication is taking place over HTTPS. 
  
As we know that the binary opens what seems to be a reverse shell on port 8443, we head over
...
This shell shows garbled output, because we do not have the right certificate
to properly connect to this shell. 

As we have now answered all the questions related to this binary, this concludes challenge 1. 
